...
@objc(ApplePayGooglePay) class ApplePayGooglePay : CDVPlugin, PKPaymentAuthorizationViewControllerDelegate {
    ...
    @objc(canMakePayments:) func canMakePayments(command: CDVInvokedUrlCommand){
      ...
      let result = CDVPluginResult(status: CDVCommandStatus.ok, messageAs: canMakePayments)
    ...
    private func failWithError(_ error: String){
      let result = CDVPluginResult(status: CDVCommandStatus.error, messageAs: error)
    ...
    func paymentAuthorizationViewController(_ controller: PKPaymentAuthorizationViewController, didAuthorizePayment payment: PKPayment, handler completion: @escaping (PKPaymentAuthorizationResult) -> Void) {
      ...
      let result = CDVPluginResult(status: CDVCommandStatus.ok, messageAs: applePaymentString)
    ...
    func paymentAuthorizationViewControllerDidFinish(_ controller: PKPaymentAuthorizationViewController) {
        if !successfulPayment {
          let result = CDVPluginResult(status: CDVCommandStatus.error, messageAs: "Payment cancelled")
          commandDelegate.send(result, callbackId: paymentCallbackId!)
        }
